				...
				while ((readNext = inspector.readNext()) != null) {
					...
					if (id.equals(readNext[0])) {
						if (pw.equals(readNext[1])) {
							int result = JOptionPane.showConfirmDialog(null, "탈퇴하시겠습니까?", "안내", JOptionPane.YES_NO_OPTION);
							...
							if (pw.equals(readNext[1])) {
								...
								if(StringUtils.isEmpty(newpw)) {
									...
									if(newpw.equals(checkpw)) {
										...
										JOptionPane.showMessageDialog(null, "비밀번호가 변경되었습니다.", "안내", JOptionPane.PLAIN_MESSAGE);
									}
									else {
									...
									JOptionPane.showMessageDialog(null, "아이디,비밀번호 조건을 확인하세요.", "안내", JOptionPane.PLAIN_MESSAGE);
								}
							}
						}
					}
